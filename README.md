...
- `mermaidClassTools.diagnosticsValidationMode` で診断方式を選べます
  - `full`: `parse + render` で検証し、構文エラーに加えてレンダリング時に検出されるエラーも診断します（高精度）
  - `light`: `parse` のみで検証し、主に構文エラーを高速に診断します（軽量）
- `mermaidClassTools.diagnosticsTriggerMode` で診断タイミングを選べます
  - `onChange`: 入力中（変更時）にも診断します
  - `onSave`: 保存時のみ診断します（デフォルト）
